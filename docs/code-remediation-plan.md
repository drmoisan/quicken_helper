...
- Shared helper `quicken_helper/utilities/excel_io.read_excel_df` now centralizes all pandas access, keeps monkeypatched tests working, and is used by `controllers.match_excel` and `controllers.category_match_session`.
...
- GUI layers (`merge_tab`) rely on those controllers for Excel ingestion, so no direct pandas usage remains.
...
### Phase 3 - finish protocol/data-model typing (partially complete)
